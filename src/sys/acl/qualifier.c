...
acl_qualifier_t wlibc_acl_get_qualifier(acl_entry_t entry)
{
	...
	return (acl_qualifier_t)((char *)entry + entry->sid);
}
...
int wlibc_acl_set_qualifier(acl_entry_t entry, const acl_qualifier_t qualifier)
{
	...
	RtlCopySid(entry->size - offsetof(struct _wlibc_acl_entry_t, sid), (acl_qualifier_t)((char *)entry + entry->sid), qualifier);
	return 0;
}
